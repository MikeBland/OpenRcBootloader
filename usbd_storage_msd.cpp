...
#else
#ifdef PCBX9LITE
    *block_num  = EESIZE/BLOCKSIZE + 3 + 2048 ;
#else
...
#else
 #if defined(PCBX12D) || defined(PCBX10)
    0x03, 0x10, // Total sectors = 4099
#else
#ifdef PCBX9LITE
		(EESIZE/BLOCKSIZE)+3, 0x08, // Total sectors = 2048+131=2179
#else
...
const char g_FAT[BLOCKSIZE] =
{
    ...
    0x03, 0x37, 0x80, 0x03, 0x39, 0xA0, 0x03, 0x3B, 0xC0, 0x03, 0x3D, 0xE0, 0x03, 0x3F, 0x00, 0x04,
		...
		0xC0, 0x0E, 0xED, 0xE0, 0x0E, 0xEF, 0x00, 0x0F, 0xF1, 0x20, 0x0F, 0xF3, 0x40, 0x0F, 0xF5, 0x60,
		0x0F, 0xF7, 0x80, 0x0F, 0xF9, 0xA0, 0x0F, 0xFB, 0xC0, 0x0F, 0xFD, 0xE0, 0x0F, 0xFF, 0x00, 0x10,
		0x01, 0x21, 0x10, 0x03, 0x41, 0x10, 0x05, 0x61, 0x10, 0x07, 0x81, 0x10, 0x09, 0xF1, 0xFF, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
		...
		if ( sector < 4099 )
 		{
			...
			while ( count )
			{
				for ( i = 0 ; i < 2 ; i += 1 )
				{
					if ( address >= 0x08020000 )		// Protect bootloader
					{
						if ( address <= (0x08000000 + (512*4096) - 256) )		// in range
						{
							program( (uint32_t *)address, (uint32_t *) buffer ) ;	// size is 256 bytes
						}
					}
